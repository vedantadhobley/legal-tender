...
- [ ] **Phase 4 of `committee_receipts` parallel UPSERT.** ~25K committees in a sequential UPSERT loop. The simple win: split the all_cmte_ids set into N chunks, each chunk written by a thread. ArangoDB UPSERT is per-document atomic so cross-chunk writes don't conflict. Probably another 2-3× on this asset.
