...
- [ ] **`WhatsApp LLC` $10M IE Oppose against Trump (and similar) — IE attribution doesn't partition by direction.** Diagnosed 2026-05-09. `whale_corporate_links.amount` is the donor's *total* contribution across all PACs they ever funded ($27.6M for KOUM, $43.2M for MELLON, etc.). When `candidate_funding`'s IE trace projects whale donations onto IE Support/Oppose, it doesn't track which side the donor's money actually flowed to — so a single donor's total can produce big IE-Oppose attributions on candidates they likely supported. Same root issue produces dual Pan Am entries for Mellon ("Pan Am Systems" + "Pan Am Railways" each get the full $43M, not split). Fix path: either (a) at `whale_corporate_links` build time, partition the donor's amount by which PACs they actually funded and what those PACs spent on; (b) at IE trace time in `candidate_upstream.py:trace_ie_sources`, look up the specific path through which whale money reached the IE-spending PAC and only attribute the corresponding portion. Non-trivial — touches the whale→corporate model in a structural way.

- [ ] **Whale → corporate attribution labels are interpretively misleading.** "Pan Am Railways: $47M IE Support for Trump" reads like a corporation funded the support, when really it's Timothy Mellon (Pan Am founder) personally donating. Same for "Marvel Entertainment" (Perlmutter), "Budget Suites of America" (Bigelow), "WhatsApp LLC" (Koum). Documented as a known model choice in funding-channels.md but the labels still surprise readers. Output-shape fix: in `candidate_upstream.py` `by_organization` cross-cut, add a `_via` field showing the donor name(s) producing the corporate attribution, and/or rename the column header so readers know they're seeing founder-personal attribution rolled up to the corporate identity. Lower priority than the partitioning bug above (#2) since this is presentation; the partitioning bug is correctness.
