...
finish() {
  ...
  if test -r /sys/fs/cgroup/memory.events; then cp /sys/fs/cgroup/memory.events memory-events.txt; fi
  printf '%s\n' "$result" > run.exit
}
trap finish EXIT
...
{ time LT_WINDOW_OUTPUT=/audit/result.json ./window.test -test.run '^TestWindowSpendingLiveGate$' -test.v > run.log 2>&1; } 2> timing.txt
printf '0\n' > gate.exit
...
{ time LT_WINDOW_OUTPUT=/audit/replay.json ./window.test -test.run '^TestWindowSpendingLiveGate$' -test.v > replay.log 2>&1; } 2> replay-timing.txt
printf '0\n' > replay.exit
