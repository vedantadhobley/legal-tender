"""House/Senate Candidate Summary Asset — parse FEC webl.zip files.

webl is FEC's per-House/Senate-candidate aggregation. Identical schema to
weball (30 fields), but scoped to congressional candidates only. Used as
ground-truth validation for our computed funding_channels totals.
"""
...
@asset(
    name="webl",
    description="FEC House/Senate candidate summary file (webl.zip) — per-candidate aggregated totals",
    group_name="fec",
...
def webl_asset(
    ...
    """Parse webl.zip files into fec_{cycle}.webl collections."""
    ...
    with arango.get_client() as client:
        for cycle in config.cycles:
            ...
            try:
                ...
                collection = arango.get_collection(db, "webl")
                ...
                if not config.force_refresh and should_restore_from_dump("webl", zip_path, "fec", cycle):
                    context.log.info("   🚀 Restoring from dump...")
                    result = restore_collection_from_dump(
                        db=db, collection_name="webl", dump_type="fec",
                ...
                with zipfile.ZipFile(zip_path) as zf:
                    ...
                    with zf.open(txt_files[0]) as f:
                        for line in f:
                            ...
                            if not decoded:
                                continue
                            record = schema.parse_line("webl", decoded)
                ...
                if batch:
                    ...
                    context.log.info(
                        f"   ✅ {cycle}: {len(batch):,} House/Senate summaries "
                    ...
                    create_collection_dump(
                        db=db, collection_name="webl", source_file=zip_path,
