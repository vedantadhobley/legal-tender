...
func (client *arangoClient) ensureDatabase(ctx context.Context, database string) error {
	if !strings.HasPrefix(database, "lt_probe_") {
		return fmt.Errorf("refusing non-probe database %q", database)
	}
	...
	if err := client.doJSON(ctx, "_system", http.MethodPost, "/_api/database", nil, request, nil); err != nil {
		...
		return fmt.Errorf("create probe database: %w", err)
	}
	return nil
}
...
func (client *arangoClient) ensureGraph(ctx context.Context, database string) error {
	request := struct {
		...
		EdgeDefinitions: []edgeDefinition{
			{Collection: relationshipsCollection, From: []string{entitiesCollection}, To: []string{entitiesCollection}},
			{Collection: receiptsCollection, From: []string{entitiesCollection}, To: []string{entitiesCollection}},
		},
		Orphans: []string{},
	}
