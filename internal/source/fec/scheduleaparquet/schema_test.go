...
func TestPhysicalSchemaMatchesMachineContract(t *testing.T) {
	...
	path := filepath.Join(repositoryRoot(t), "contracts", "facts", "fec", "schedule-a", "columnar", "v1", "physical-schema.json")
	...
	if contract.SchemaVersion != PhysicalSchemaVersion || contract.ColumnCount != schema.ColumnCount() || contract.SourceColumns.Count != schedulea.FieldCount {
	...
	wantAdded := []string{
		...
		ColumnNormalizationState, ColumnNormalizationIssuesJSON,
		ColumnReceiptAmountMinorUnits, ColumnReceiptAmountSourceScale, ColumnReceiptAmountState,
		ColumnAggregateYTDMinorUnits, ColumnAggregateYTDSourceScale, ColumnAggregateYTDState,
		ColumnReceiptDate, ColumnReceiptAtLocal, ColumnPublisherLoadedAtLocal,
		ColumnFECElectionYear, ColumnReportYear, ColumnTwoYearTransactionPeriod, ColumnMemoedSubtotal,
	}
